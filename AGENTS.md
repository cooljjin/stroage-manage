...
AI 코딩 에이전트의 저장소 운영 지침입니다. 작업 전에 반드시 이 파일과 `README.md`를 읽습니다.

## 적용 범위

- 저장소 전체에 적용하며 하위 `AGENTS.md`가 있으면 그 범위에서는 하위 지침을 우선합니다.
- 사용자 요청을 우선하되 위험하거나 기존 동작을 크게 바꾸면 먼저 이유를 설명하고 확인합니다.
...
- 계정: 이메일/비밀번호·OAuth 로그인, 초대코드 기반 매장 편입, 직원/매장/master 관리
- 재고: 상품 등록/수정, 재고 현황 조회, 바코드 스캔, 입고·출고·이동·조정·메모, 작업 로그와 복원
- 발주: 부족 재고·발주 관리, 발주처의 링크·문자 발주, 카테고리/단위 관리
- 업무: 홈 대시보드의 할 일·인수인계·입고 확인, 프랩 품목 관리·프랩 모드, 단체 주문 계산
...
- 요청 범위만 작게 수정하고 기존 동작·파일 스타일·패턴을 보존합니다.
- UI, DB 구조, 인증 흐름, 라우팅, 쿼리 의미를 임의로 바꾸지 않습니다.
- 반복 쿼리·로직을 정리하기 전에 사용 범위를 확인합니다.
- 타입 안정성을 유지하고 불필요한 `any`를 쓰지 않습니다.
- 사용자 변경을 되돌리거나 무관한 포맷팅·정렬 변경을 하지 않습니다.
...
- DB 구조는 사용자가 명시한 경우에만 변경합니다.
- 스키마 변경은 `supabase/migrations/`에 새 파일로 추가합니다. 기존 migration은 이미 적용됐을 수 있으므로 수정하지 않습니다.
- `src/types/supabase.ts`, `src/types/domain.ts`도 함께 맞춥니다.
- RLS/policy/RPC 변경의 앱 코드 영향 범위를 확인합니다.
- `supabase/sql/`의 SQL patch보다 새 migration을 우선합니다.

## 인증과 OAuth 규칙

인증은 `AuthService`만 사용합니다. 지원 흐름은 이메일/비밀번호 로그인·회원가입과 Google·Kakao·Apple OAuth입니다.

OAuth 네이티브 앱 콜백:

> **확인 필요:** 아래 단일 callback 안내는 [iOS TestFlight 배포 채널](#ios-testflight-배포-채널)과 함께 확인합니다. `AuthService.ts`는 두 채널 프로토콜을 허용하며 실제 Redirect URLs·서명 설정은 별도 확인 대상입니다.
...
초대는 이메일 링크가 아니라 로그인 후 초대코드 입력 방식입니다. 로그인·회원가입 후 프로필이 없으면 매장 연결 화면에서 선택합니다.

- `새 매장 만들기`: 성공하면 해당 매장의 `store_admin`
- `초대코드 입력`: 성공하면 초대한 매장으로 편입
...
상품은 `products`, 수량은 `inventory`에 있습니다. `normalizeInventoryItem`이 `warehouse_qty`, `store_qty`, `total_stock`, `is_low_stock`을 계산합니다.
...
- 버튼 텍스트: `발주품목 추가`
- 검색어가 없으면 카테고리만 표시하고, 누르면 해당 품목 체크 목록을 표시합니다. 검색어가 있으면 카테고리 분류 없이 품목을 바로 표시합니다.
- 각 품목에 발주품목·`긴급` 체크를 두며, `긴급`을 켜면 발주품목에 자동 포함합니다.
...
- 카테고리는 기본 한 줄 가로 스크롤이며, 오른쪽의 아래쪽 삼각형 버튼으로 전체 카테고리의 여러 줄 펼침/접힘을 전환합니다.
...
홈은 대시보드 날짜, 입고 로그 또는 입고 확인 품목, 할 일, 인수인계, 삭제/복원 상태를 표시합니다.
...
- 단위 변환·소수 수량을 보존하고 `unit_weight`, `unit_weight_unit`, `processed_unit_weight`, `processed_unit_weight_unit`을 임의로 단순화하지 않습니다.
- 계산 결과가 재고와 연결되므로 수량 단위 변경 시 관련 helper·타입을 함께 확인합니다.
...
- 기존 Tailwind 패턴과 조용하고 실용적인 업무용 UI를 유지합니다. 장식적 레이아웃·마케팅형 히어로는 만들지 않고 카드 중첩은 피합니다.
- 모바일 버튼/입력/테이블 겹침과 버튼 텍스트 잘림을 방지합니다.
...
- 브라우저·Simulator·실기기에서 수행하는 UI/데이터 검증은 전용 **`테스트 매장`**에서만 진행합니다.
- 테스트 계정 로그인은 Aside vault의 `Stockly 테스트 계정` 항목 자동완성만 사용합니다. 이메일·비밀번호·복구 코드·토큰은 이 파일, 소스, 테스트, 문서, 로그, 채팅에 기록하지 않습니다.
- 검증 전에 현재 선택된 매장이 `테스트 매장`인지 확인합니다. 확인할 수 없으면 운영 매장의 데이터를 조회·수정하지 않습니다.
- 운영 매장 접근·조회·수정은 사용자가 명시적으로 요청한 경우에만 허용합니다.
...
모든 코드 변경 후 [기본 검증](#주요-명령어), Supabase 직접 호출 변경 후 [잔여 검색](#supabase-의존성-규칙)을 실행합니다.
...
- 요청 없이 DB 스키마·Supabase 설정·라우팅 구조·인증 흐름 변경 또는 UI 전면 개편
...
완료 보고에는 변경 내용·주요 파일·동작 차이, `npm run build`·`npm run lint` 결과, 미실행 검증과 이유를 포함합니다.

## 사용자와의 협업 톤

한국어로 간결·명확하게 설명하고 기술적 결정에는 이유를 붙입니다. 문제의 원인·영향을 숨기지 않으며 가능 여부보다 실제 변경 내용을 보고합니다.
