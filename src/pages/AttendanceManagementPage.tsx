...
export function AttendanceManagementPage({ currentStoreId, currentRole }: Props) {
  ...
  return (
    <section>
      <PageTitle title="근태관리" description="NFC 실제 태그 시각과 급여 반영 시각을 분리해 관리합니다." action={<button type="button" className="secondary-button inline-flex items-center gap-2" onClick={() => void loadData()} disabled={loading}><RefreshCw size={17} />새로고침</button>} />
      ...
      <div className="mb-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-6">
        <label className="text-sm font-semibold">조회 시작<input type="date" className="field mt-1" value={fromDate} onChange={(event) => setFromDate(event.target.value)} /></label>
        <label className="text-sm font-semibold">조회 종료<input type="date" className="field mt-1" value={toDate} onChange={(event) => setToDate(event.target.value)} /></label>
